...
hid_device* get_handle(hid_usage_info* usage_info) {
    ...
    if (!usage_info) {
        write_log(_ERROR, "RAWHID - Usage info is NULL");
        return NULL; // Return NULL to indicate failure
    }

    write_log_format(_INFO, "RAWHID - Attempting to open HID device with Vendor ID: 0x%x, Product ID: 0x%x",
    ...
    if (!handle) {
        write_log_format(_ERROR, "RAWHID - Failed to get handle for Vendor ID: 0x%x, Product ID: 0x%x",
            usage_info->vendor_id, usage_info->product_id);
        return NULL; // Return NULL to indicate failure
    }

    // Log success and return the handle
    write_log_format(_INFO, "RAWHID - Successfully got handle for Vendor ID: 0x%x, Product ID: 0x%x",
        usage_info->vendor_id, usage_info->product_id);
    return handle;
}
...
void open_usage_path(hid_usage_info* usage_info, hid_device** handle) {
    ...
    if (!handle || !usage_info) {
        write_log(_ERROR, "RAWHID - Invalid arguments");
        return;
    }

    // Check if handle is NULL
    if (*handle == NULL) {
        write_log(_ERROR, "RAWHID - No handle to open");
        return;
    }

    write_log_format(_INFO, "RAWHID - Enumerating HID devices for Vendor ID: 0x%x, Product ID: 0x%x",
        usage_info->vendor_id, usage_info->product_id);
    ...
    if (!enum_device_info) {
        write_log_format(_ERROR, "RAWHID - Failed to enumerate devices for Vendor ID: 0x%x, Product ID: 0x%x",
            usage_info->vendor_id, usage_info->product_id);
        return;
    }
    ...
    for (; enum_device_info != NULL; enum_device_info = enum_device_info->next) {
        if (enum_device_info->usage_page == usage_info->usage_page &&
            ...
            *handle = hid_open_path(enum_device_info->path);
            if (*handle) {
                write_log_format(_INFO, "RAWHID - Successfully opened device with Usage Page: 0x%x, Usage: 0x%x",
                    usage_info->usage_page, usage_info->usage);
                break;
            }
            else {
                write_log_format(_ERROR, "RAWHID - Failed to open device with Usage Page: 0x%x, Usage: 0x%x",
                    usage_info->usage_page, usage_info->usage);
            }
        }
    }
...
int write_to_handle(hid_device** handle, unsigned char* message, size_t size) {
    ...
    write_log_format(_DEBUG, "RAWHID - Attempting to write %d bytes to handle", size);

    // Write the message to the HID device
    int result = hid_write(handle, message, size);
    if (result < 0) {
        write_log(_ERROR, "RAWHID - Failed to write to handle");
        return result;
    }

    write_log(_DEBUG, "RAWHID - Wrote to handle", message);
