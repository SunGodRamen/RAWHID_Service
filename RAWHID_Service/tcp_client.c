...
SOCKET init_client(tcp_socket_info* server_info) {
    write_log(_DEBUG, "TCP Client - Entering init_client");

    WSADATA wsaData;  // WinSock Data

    // Initialize WinSock
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        write_log_format(_ERROR, "TCP Client - Failed to initialize WinSock. Error Code: %d", WSAGetLastError());
        return INVALID_SOCKET;
    }

    // Check for null server info
    if (!server_info) {
        write_log(_ERROR, "TCP Client - Server information is NULL");
        WSACleanup();
        return INVALID_SOCKET;
    }
    ...
    if (clientSocket == INVALID_SOCKET) {
        write_log_format(_ERROR, "TCP Client - Failed to create socket. Error Code: %d; Server IP: %s, Port: %d",
    ...
    if (inet_pton(AF_INET, server_info->ip, &(serverAddr.sin_addr)) <= 0) {
        write_log_format(_ERROR, "TCP Client - Invalid IP address or error in inet_pton. Error Code: %d; Server IP: %s, Port: %d",
            WSAGetLastError(), server_info->ip, server_info->port);
    ...
    if (connect(clientSocket, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) {
        write_log_format(_ERROR, "TCP Client - Connect failed. Error Code: %d; Server IP: %s, Port: %d",
            WSAGetLastError(), server_info->ip, server_info->port);
        cleanup_client(clientSocket);
        return INVALID_SOCKET;
    }

    write_log(_INFO, "TCP Client - Successfully connected to the server");
...
int read_message_from_server(SOCKET serverSocket, char* buffer) {
    write_log(_DEBUG, "TCP Client - Entering read_message_from_server");
    // Check for null buffer
    if (!buffer) {
        write_log(_ERROR, "TCP Client - Buffer is NULL");
        return -1;
    }
    ...
    while (totalBytesRead < MESSAGE_SIZE_BYTES) {
        ...
        if (bytesRead == SOCKET_ERROR && WSAGetLastError() != 10053) {
            write_log_format(_ERROR, "TCP Client - Error occurred while reading from socket. Error Code: %d", WSAGetLastError());
            return -1;
        }

        // Check for server disconnection
        if (bytesRead == 0) {
            write_log(_ERROR, "TCP Client - Server disconnected before sending full message.");
    ...
    write_log_format(_DEBUG, "TCP Client - Read %d bytes from server", totalBytesRead);
    write_log_byte_array(_DEBUG, buffer, totalBytesRead);
    write_log(_DEBUG, "TCP Client - Exiting read_message_from_server");
    return totalBytesRead;
}
...
int send_to_server(SOCKET serverSocket, const char* data, int dataLength) {
    write_log(_DEBUG, "TCP Client - Entering send_to_server");
    // Check for null data or zero length
    if (!data || dataLength <= 0) {
        write_log(_ERROR, "TCP Client - Invalid data to send");
        return -1;
    }

    // Send the data
    if (send(serverSocket, data, dataLength, 0) == SOCKET_ERROR) {
        write_log_format(_ERROR, "TCP Client - Failed to send data. Error Code: %d", WSAGetLastError());
        return -1;
    }

    write_log_format(_DEBUG, "TCP Client - Sent %d bytes to server:", dataLength);
    write_log_byte_array(_DEBUG, data, dataLength);
...
void cleanup_client(SOCKET serverSocket) {
    write_log(_DEBUG, "TCP Client - Entering cleanup_client");
    ...
    // Cleanup WinSock
    WSACleanup();
    write_log(_INFO, "TCP Client - Client resources cleaned up");
    write_log(_DEBUG, "TCP Client - Exiting cleanup_client");
}
